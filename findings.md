...
## Phase 3(bugfix_plan v1.1)の記録

- P3-L3(N-5)per-file-ignore の計画/実環境の食い違い(D-21・要人間確認): 計画 P3-L3 Green は「ruff の per-file-ignore(flask_helpers)から不要になった無効化を削除して床を回復」「§6: 修正後に **F821** が再発しないことを ruff が機械保証」と記すが、実 `ruff.toml` の flask_helpers エントリは **`["F401"]`(F821 ではない)**。F821(未定義名)はそもそも ignore に無く、`from libcommon.color import *`(L26 スター import)により **ruff が抑止**している(E-10 既知限界=スター import ファイルの F405 化。計画 §6 も範囲外と明記)。よって「F821 ignore の削除」は対象が存在せず不能。F401 ignore は依然必要 — flask_helpers には本修正のスコープ外の**既存の未使用 import が9件**残る(`http_errors` の UnexpectedAPIErrorFormat/ForbiddenAPIErrorFormat/ResourceNotFoundAPIErrorFormat/BadRequestAPIErrorFormat/UnauthorizedAPIErrorFormat/RateLimitExceededAPIErrorFormat、`InvalidEmailFormatErrorFormat`、functools `partial`、`Locale`)。これらを消すのは範囲外の「ついで修正」(禁止)。**判断: per-file-ignore は不変のまま、床は degrade せず(F821 は従前どおりスター import で抑止)。核の修正(NameError→既存フォーマット族)は計画どおり完了。** 修正の到達形: L167 `handle_query_param_errors` を ValidationErrorsFormat(400・'validation_error' ロケール文言)で新設 / L208 `MinLengthNotReachedErrorFormat` は validation_errors.py L19 実在につき import 追加のみ / L295–314 `ErrorCode` import 追加、L298 の未定義 `locale` は署名変更(=quantz の `@google_oauth_token_check` 消費を破壊)を避け、構築済み `GoogleOauthTokenErrorFormat` を既存 `.http_response()`(401)で短絡へ変更。名指しゴールデン `decorators/required_query_missing_nameerror`(NameError 凍結)を `decorators/required_query_missing`(400 検証エラー応答)へ差し替え。
...
- `web/session.py` の旧 `Session.clear()` は、現在の `session:{sid}` と `user_id:{sid}` だけを削除する一方、
  `sessions:{user_id}` 集合全体を削除していた。そのため、同じユーザーの別端末 Session 本体を残したまま
  全端末の逆引きだけが失われる不整合があった。
- canonical 内の Python 呼び出しを検索した結果、`Session.clear()` の利用は `Session.start()` と
  `tests/test_char_session.py` に限られていた。外部consumerとの後方互換のため `clear()` は削除せず、
  現在の Session だけを削除する `clear_current()` の alias として残す。
- 全端末失効は `revoke_all(user_id)` に分離し、`sessions:{user_id}` から各 sid の
  `session:{sid}` / `user_id:{sid}` を削除してから集合自体を削除する。
